...
@section('container')
<h1 class="text-3xl font-black text-center md:text-left mb-5">Edit Article</h1>
<div class="min-h-full">
    <div class="max-w-full w-full space-y-8">
        <form class="space-y-6" action="/dashboard/articles/{{ $article->slug }}" method="POST" novalidate enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="rounded-md max-w-lg">
                <div class="mb-2">
                    <label for="title" class="block text-sm font-medium text-gray-700 mb-2">Title</label>
                    <input id="title" name="title" type="title" autocomplete="title" class="focus:ring-indigo-500 focus:border-indigo-500 p-3 flex-1 block w-full rounded-md sm:text-sm border-gray-300 border" value="{{ old('title', $article->title) }}" autofocus>
                    @error('title')<small class="text-red-600 font-medium block my-2">{{ $message }}</small>@enderror
                </div>
                <div class="mb-2">
                    <label for="slug" class="block text-sm font-medium text-gray-700 mb-2">Slug</label>
                    <input id="slug" name="slug" type="text" readonly autocomplete="slug" class="p-3 flex-1 block w-full rounded-md sm:text-sm border-gray-300 border bg-gray-100" value="{{ old('slug', $article->slug) }}">
                ...
                <div class="mb-2">
                    ...
                    <textarea id="description" name="description" type="description" autocomplete="description" class="focus:ring-indigo-500 focus:border-indigo-500 p-3 flex-1 block w-full rounded-md sm:text-sm border-gray-300 border">{{ old('description', $article->excerpt) }}</textarea>
                ...
                <div class="mb-2">
                    ...
                    <select id="tag" name="category_id" autocomplete="tag-name" class="focus:ring-indigo-500 focus:border-indigo-500 p-3 flex-1 block w-full rounded-md sm:text-sm border-gray-300 border">
                        @foreach($tags as $tag)
                ...
                <div class="mb-2">
                    ...
                    @if($article->image)
                    <img class="rounded-md border border-gray-200 mb-3 overflow-hidden" id="img-preview" src="{{ asset('storage/' . $article->image) }}" alt="">
                    @else
